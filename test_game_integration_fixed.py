...
def test_dice_game(telegram_id1, telegram_id2):
    ...
    # Бросок костей первого игрока
    dice_data = {"dice_count": 2}
    response = requests.post(f"{BASE_URL}/api/dice/play/{room_id}?telegram_id={telegram_id1}", json=dice_data)
    if response.status_code != 200:
        print(f"❌ Ошибка броска костей игрока 1: {response.text}")
        return False
    
    roll_result = response.json()
    print(f"🎲 Результат броска игрока 1: {roll_result}")
    ...
    if response.status_code != 200:
        ...
        print("ℹ️ Возможно, игра уже завершилась после первого броска")
    else:
        roll_result2 = response.json()
        print(f"🎲 Результат броска игрока 2: {roll_result2}")
...
def test_rps_game(telegram_id1, telegram_id2):
    ...
    result = response.json()
    print(f"✂️ Результат RPS: {result}")
